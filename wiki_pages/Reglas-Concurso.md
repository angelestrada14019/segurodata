...
- [ ] README.md con instrucciones de instalación + URL Vercel (frontend) + URL Cloud Run (backend)
...
→ El Atlas del Crimen es análisis descriptivo — explica qué ha pasado históricamente. Este sistema tiene tres capas operacionales: descriptivo (mapa deck.gl en tiempo real), predictivo (XGBoost + SHAP + ruptures para cambios estructurales por UPZ), y prescriptivo real (tabla ontológica SHAP→entidad→acción con nombre del CAI). El Atlas operó a nivel departamental sin modelo ML ni capa prescriptiva. Este proyecto opera a nivel UPZ con API REST pública (FastAPI + Cloud Run, serverless).
