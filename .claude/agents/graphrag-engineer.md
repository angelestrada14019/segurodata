...
4. **OpenRouter**: modelo desde env `LLM_MODEL` (default `google/gemini-flash-1.5`), fallback `LLM_MODEL_FALLBACK` (claude-haiku) ante 429/5xx. TODA llamada pasa por el TTLCache (key = sha256(model+prompt normalizado)). La cuota gratis es 1M tokens/día — el caché no es opcional.
